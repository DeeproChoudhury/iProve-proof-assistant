...
export class LogicInterfaceQueue {
  ...
  queueEntails(reasons: AST.Line[], goal: AST.Line, cb: (outcome: ProofOutcome) => void, strip_types: boolean = false) {
    ...
    this.queue(() => this.li.entails(deepCurrent(reasons), deepCurrent(goal), strip_types).then(cb));
  }
}
